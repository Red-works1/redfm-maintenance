...
window.REDFM = (function () {
  ...
  // Generic id-preserving read + patch. listItems() drops the SharePoint item id, which is
  // fine for read-only views but useless for anything that has to be closed out later —
  // an RDM alarm, for instance, is created while it is still ringing and updated when it clears.
  async function listItemsWithId(title, filterFn) {
    const sid = await getSiteId();
    const lid = await getListId(title);
    let url = "/sites/" + sid + "/lists/" + lid + "/items?$expand=fields&$top=500";
    const out = [];
    while (url) {
      const page = await graph(url);
      page.value.forEach(i => { const f = i.fields || {}; f._id = i.id; if (!filterFn || filterFn(f)) out.push(f); });
      url = page["@odata.nextLink"] ? page["@odata.nextLink"].replace("https://graph.microsoft.com/v1.0", "") : null;
    }
    return out;
  }

  async function updateItem(title, id, fields) {
    const sid = await getSiteId();
    const lid = await getListId(title);
  ...
  const RED = [0xC8, 0x10, 0x2E];      // RED FM brand red
  const INK = [0x1E, 0x1E, 0x1E];      // RED FM brand near-black
  ...
  // Draw the RED FM logo lockup as vector: solid red square with RED reversed out of it,
  // then FM alongside in near-black. Vector rather than a bitmap so it stays crisp at any
  // size and adds nothing to the file weight. Proportions follow the brand artwork
  // (square mark, FM set to the right at roughly half the mark's width). Returns bottom Y.
  function drawLogo(doc, x, y, w) {
    const s = Math.min(w / 1.62, 54);   // side of the red square, capped so the header stays calm
    doc.setFillColor.apply(doc, RED);
    doc.rect(x, y, s, s, "F");
    doc.setFont("helvetica", "bold");
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(s * 0.42);
    doc.text("RED", x + s / 2, y + s * 0.665, { align: "center" });
    doc.setTextColor.apply(doc, INK);
    doc.setFontSize(s * 0.42);
    doc.text("FM", x + s * 1.14, y + s * 0.665);
    return y + s;
  }
  ...
  function stampFooter(doc, fgasNo) {
    ...
    for (let p = 1; p <= n; p++) {
      ...
      doc.setTextColor.apply(doc, INK); doc.text("The work that ", 40, H - 26);
  ...
  function buildReportPdf(meta, sections) {
    ...
    doc.setTextColor.apply(doc, INK); doc.setFont("helvetica", "bold"); doc.setFontSize(16);
